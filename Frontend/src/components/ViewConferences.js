...
function ViewConferences() {
    ...
    const onFormSubmit = async (data) => {
        ...
        console.log(data);
        await axios.post("http://localhost:8080/conference/filters", data)
            .then((res) => {
                setConferences(res.data.payload)
                if (res.data.payload.length === 0) {
                    Alert("warning", "No Conferences")
                }
                else
            ...
            <table className="mt-6 bg-[#ede0d4] bg-glass rounded-3xl p-1 w-full shadow-2xl">
                <thead className="bg-[#ede0d4] border-b-2 border-black rounded-3xl">
                    <tr>
                        ...
                        <th className="p-4 text-[1.1rem] font-Alkatra tracking-wide text-left" >Title Of Conference</th>
                        <th className="p-4 text-[1.1rem] font-Alkatra tracking-wide text-left" >Authors List</th>
                        <th className="p-4 text-[1.1rem] font-Alkatra tracking-wide text-left" >Conference Name</th>
                        <th className="p-4 text-[1.1rem] font-Alkatra tracking-wide text-left" >DOI Number</th>
                        ...
                        <th className="p-4 text-[1.1rem] font-Alkatra tracking-wide text-left" >Pages of Conference</th>
                ...
                <tbody>
                    {conferences.length === 0 &&
                        <tr className="text-4xl text-center font-bold"><td className="text-red-400" colSpan="10">No COnferences</td></tr>}
                    ...
                    {conferences !== [] &&
                        conferences.map((conference, key) => {
                            ...
                            `}>
                                    ...
                                    <td className="p-4 text-sm text-grey-700">{conference.title}</td>
                                    ...
                                    <td className="p-4 text-sm text-grey-700">{conference.conferenceName}</td>
                                    <td className="p-4 text-sm text-grey-700">{conference.doi}</td>
                                    <td className="p-4 text-sm text-grey-700">{conference.impactFactor}</td>
                                    <td className="p-4 text-sm text-grey-700">{
                                    ...
                                    <td className="p-4 text-sm text-grey-700">{conference.publicationDate}</td>
                                    <td className="p-4 text-sm text-grey-700">{conference.volume}</td>
                                    <td className="p-4 text-sm text-grey-700">{
